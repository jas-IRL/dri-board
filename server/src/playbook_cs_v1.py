"""CS Readiness Playbook (v1)

This is a code-encoded representation of the private playbook content used for:
- Bridge Mode seeding (readiness elements)
- Claude-backed recommendation prompts

Source: CS Readiness Playbook.pdf (2026-03-19)
"""

PLAYBOOK_VERSION = "2026-03-19"
...
def playbook_prompt_block() -> str:
    """Return a compact prompt block to include in LLM prompts."""
    lines = []
    lines.append(f"PLAYBOOK_VERSION: {PLAYBOOK_VERSION}")
    lines.append("TENETS:")
    for t in TENETS:
        lines.append(f"- {t}")
    lines.append("P_LEVELS:")
    for k, v in P_LEVELS.items():
        outs = ", ".join(v.get("outputs") or [])
        lines.append(f"- {k}: {v['label']} | rigor={v['rigor']} | outputs={outs}")
    lines.append("MODES:")
    for m in MODES:
        lines.append(f"- {m['name']}: used for {m['usedFor']}")
    lines.append("READINESS_ELEMENTS (name | what_it_covers | ready_when):")
    for e in READINESS_ELEMENTS:
        lines.append(f"- {e['name']} | {e['whatItCovers']} | {e['readyWhen']}")
    return "\n".join(lines)
